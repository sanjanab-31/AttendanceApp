...
export default function AttendanceHistory() {
  ...
  const filteredAttendance = attendance
    .filter((record: any) => {
      ...
      const matchesToDate =
        !to || recordDate <= new Date(to.getFullYear(), to.getMonth(), to.getDate(), 23, 59, 59, 999);

      return matchesSearch && matchesEmployee && matchesFromDate && matchesToDate;
    })
    ...
    <View className="bg-white px-4 py-3 rounded-2xl shadow-sm mb-2 border border-gray-100">
      <View className="flex-row justify-between items-start">
        <View className="flex-1 pr-3">
          <Text className="text-base font-bold text-gray-900">{item.employeeName}</Text>
          <Text className="text-xs text-gray-500 mt-1">{formatAttendanceDate(item.date)}</Text>
        </View>
        <Text className="text-lg font-bold text-blue-700">{formatCurrency(item.totalSalary || 0)}</Text>
      </View>

      <View className="flex-row mt-3 rounded-xl bg-slate-50 px-3 py-2.5">
        <View className="flex-1">
          <Text className="text-[11px] uppercase font-bold text-gray-400">Shift</Text>
          <Text className="text-sm font-semibold text-gray-700">{item.shiftHours || 0}h</Text>
        </View>
        <View className="flex-1">
          <Text className="text-[11px] uppercase font-bold text-gray-400">OT</Text>
          <Text className="text-sm font-semibold text-gray-700">{item.otHours || 0}h</Text>
        </View>
        <View className="flex-1">
          <Text className="text-[11px] uppercase font-bold text-gray-400">Rate</Text>
          <Text className="text-sm font-semibold text-gray-700">
            ₹{employees.find((employee: any) => employee.employeeId === item.employeeId)?.hourlyRate ?? 0}/hr
          </Text>
        </View>
      </View>

      <View className="mt-3 flex-row items-center justify-between">
        <Text className="text-xs font-semibold uppercase tracking-wide text-gray-400">Total Salary</Text>
        <Text className="text-sm font-bold text-emerald-600">{formatCurrency(item.totalSalary || 0)}</Text>
      </View>

      <View className="mt-2 rounded-xl bg-gray-50 px-3 py-2">
        <Text className="text-[11px] uppercase font-bold text-gray-400 mb-1">Remarks</Text>
        <Text className="text-sm text-gray-600">{item.remarks?.trim() ? item.remarks : 'None'}</Text>
      </View>
    </View>
  );
  ...
  const header = (
    <View className="pb-3">
      <View className="flex-row items-center justify-between mb-3">
        <Text className="text-2xl font-bold text-gray-800">Attendance History</Text>
        <TouchableOpacity onPress={handleClearFilters} className="px-3 py-2 rounded-xl bg-gray-200">
          <Text className="text-xs font-semibold text-gray-700">Clear Filters</Text>
        </TouchableOpacity>
      </View>
      ...
      <Text className="text-xs font-bold uppercase tracking-wide text-gray-500 mb-2">Employee Filter</Text>
      ...
      <Text className="text-xs font-bold uppercase tracking-wide text-gray-500 mb-2">Date Filters</Text>
      <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-3">
        {[
      ...
      <View className="flex-row mb-3">
        <View className="flex-1 mr-2">
          <Text className="text-xs font-bold uppercase tracking-wide text-gray-500 mb-2">From Date</Text>
          <TextInput
        ...
        <View className="flex-1 ml-2">
          <Text className="text-xs font-bold uppercase tracking-wide text-gray-500 mb-2">To Date</Text>
          <TextInput
            className="bg-white border border-gray-200 px-3 py-2.5 rounded-2xl text-gray-800"
            placeholder="YYYY-MM-DD"
            value={toDate}
      ...
      <View className="flex-row items-center justify-between mb-2 px-1">
        <Text className="text-sm font-semibold text-gray-700">{filteredAttendance.length} records</Text>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="items-center justify-center rounded-3xl bg-white border border-dashed border-gray-200 px-6 py-10 mt-4">
            ...
            <Text className="text-gray-500 mt-3 text-base font-semibold">No attendance records found</Text>
            <Text className="text-gray-400 mt-1 text-center text-sm">Try another employee, search term, or date range.</Text>
          </View>
        }
      />
    </SafeAreaView>
