...
export default function Reports() {
  ...
  const filteredAttendance = attendance.filter((record: any) => {
    let match = true;
    if (selectedEmployee)
      match = record.employeeId === selectedEmployee.employeeId;
  ...
  const stats = {
    shiftHours: filteredAttendance.reduce(
      (acc: number, r: any) => acc + r.shiftHours,
      0,
    ),
    otHours: filteredAttendance.reduce(
      (acc: number, r: any) => acc + r.otHours,
      0,
    ),
    salary: filteredAttendance.reduce(
      (acc: number, r: any) => acc + r.totalSalary,
      0,
    ),
    bonus: filteredBonuses.reduce(
      (acc: number, b: any) => acc + b.bonusAmount,
      0,
    ),
  };
    ...
    <SafeAreaView edges={["top"]} className="flex-1 bg-gray-50">
      <ScrollView className="px-6 pt-6" showsVerticalScrollIndicator={false}>
        ...
        <View className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 mb-6">
          <Text className="text-gray-600 mb-3 font-medium text-sm">
            Filters
          </Text>
          ...
          >
            ...
            >
              <Text
                className={`text-xs font-bold ${selectedEmployeeId === "" ? "text-white" : "text-gray-500"}`}
              >
                All Employees
              </Text>
            </TouchableOpacity>
            {employees.map((emp: any) => (
              ...
              >
                <Text
                  className={`text-xs font-bold ${selectedEmployeeId === emp.id ? "text-white" : "text-gray-500"}`}
                >
                  {emp.name}
                </Text>
              </TouchableOpacity>
          ...
          <View className="flex-row space-x-2">
            <View className="flex-1">
              <TextInput
                className="bg-gray-50 p-3 rounded-xl text-xs border border-gray-100"
        ...
        <View className="flex-row flex-wrap mb-6">
          <StatBox
            label="Total Shift"
            value={`${stats.shiftHours} hrs`}
            color="text-blue-600"
          />
          <StatBox
            label="Total OT"
            value={`${stats.otHours} hrs`}
        ...
        <View className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden mb-10">
          <View className="bg-gray-50 p-4 border-b border-gray-100 flex-row">
            <Text className="flex-2 text-gray-400 font-bold text-[10px] uppercase">
              Employee
            </Text>
            <Text className="flex-1 text-gray-400 font-bold text-[10px] uppercase text-center">
              Hours
            </Text>
            <Text className="flex-1 text-gray-400 font-bold text-[10px] uppercase text-right">
              Amount
            </Text>
          </View>
          {filteredAttendance.length > 0 ? (
            filteredAttendance.map((item, idx) => (
              <View
                key={item.id}
                className={`p-4 flex-row ${idx !== filteredAttendance.length - 1 ? "border-b border-gray-50" : ""}`}
              >
                ...
                <Text className="flex-1 text-gray-600 text-xs text-center font-medium">
                  {item.shiftHours + item.otHours}h
                </Text>
                <Text className="flex-1 text-gray-800 text-xs font-bold text-right">
